...
export async function resetore(
...
async function findVerseIdsByChapterIds(
  chapterIds: number[]
): Promise<number[]> {
  const verses = await db.verse.findMany({
    where: {
...
async function deleteChapters(ids: number[]) {
  try {
    const verseIds = await findVerseIdsByChapterIds(ids);
    await deleteVerses(verseIds);
...
async function deleteVerses(ids: number[]) {
  try {
    ...
    console.error(
      "Failed to delete verses and associated commentaries:",
      error
    );
    throw new Error("Error deleting verses and associated commentaries.");
  }
}
