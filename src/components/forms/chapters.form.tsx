...
export const chapterFormSchema = z.object({
    ...
    bookId: z.number({ required_error: "This field is required." })
})
...
export default function ChaptersForm({ chapter, books }: { chapter?: Chapter, books: Book[] }) {
    ...
    const handleAddNew = async (data: ChapterFormSchema) => {
        const res = await apiClientActions.chapters.create(data)
    ...
    const handleUpdate = async (data: ChapterFormSchema) => {
        if (!chapter) return;
        const res = await apiClientActions.chapters.update(chapter.id, data)
        if (res.succeed && res.data) return router.push("/dashboard/chapters")
        if (res.code === "SLUG_MUST_BE_UNIQUE") {
            form.setError("slug", {
                message: definedMessages.SLUG_MUST_BE_UNIQUE
                                ...
                                <FormItem>
                                    ...
                                    <FormControl>
                                        <ComboBox
                                            ...
                                            ref={field.ref}
                                            options={books.map((book) => ({ label: book.name, value: book.id.toString() }))}
                                        />
                                    </FormControl>
                                    {
                                        fieldState.error &&
