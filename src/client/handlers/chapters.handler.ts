...
export async function getById(id: number): Promise<ApiResponse<IChapter>> {
  try {
    const res = await axios.get<ApiResponse<IChapter>>(`/api/chapters/${id}`);
    return res.data;
  } catch (error) {
    return {
      succeed: false,
...
export async function update(
  id: number,
  update: ChapterFormSchema
): Promise<ApiResponse<IChapter>> {
  try {
    const res = await axios.put<ApiResponse<IChapter>>(
      `/api/chapters/${id}`,
      update
    );
    if (res.status !== 200) throw new Error();
