...
const checkAndSaveNote = (state: ReadBookStoreType) => {
  ...
  if (
    note.noteId &&
    note.noteId === activeVerse.id &&
    note.text !== activeVerse.data?.notes?.[0]?.text
  ) {
...
const syncHighlightsWithDB = async (
  verseId: number,
...
export const useReadBookStore = create<ReadBookStoreType>()((set, get) => ({
  ...
  setActiveCommentary: (commentaryId) => {
    ...
    const activeCommentary = activeAuthor.commentaries?.list?.find(
      (c) => c.id === commentaryId
    );
    set((state) => ({
      ...state,
  ...
  setActiveVerseNote: (note) => {
    set((state) => ({
      ...
      activeVerseNote: {
        ...
        changed:
          note !== undefined || !(get().activeVerseNote.noteId && note === ""),
      },
    }));
  },
  ...
  createNewBookmark: async (verse) => {
    if (!verse) return;
    const { data: bookmark } = await clientApiHandlers.bookmarks.create(
      verse?.id
    );
  ...
  loadBookmarks: async () => {
    ...
    const { data: bookmarks } = await clientApiHandlers.bookmarks.get({
      page: 1,
      perPage: -1,
    });
    ...
    const updatedTopicsList = get().topicsList?.map((topic) => {
      if (topic.verses) {
        const verses = topic.verses.map((verse) => {
          if (verse.id === verseId) {
            const updatedHighlights =
              verse.highlights?.filter((h) => {
                if (
                  (h.text.includes(text.trim()) ||
                    text.trim().includes(h.text)) &&
                  h.index === index
                )
                  return false;
                return true;
